...
layui
		.define(
				...
				function(exports) {
					...
					var $ = layui.jquery, layer = layui.layer, form = layui
							.form(), hint = layui.hint(), device = layui
							.device()
					...
					Edit.prototype.build = function(id, settings) {
						...
						var that = this, config = that.config, ELEM = 'layui-layedit', textArea = $('#'
								+ id), name = 'LAY_layedit_' + (++that.index), haveBuild = textArea
								.next('.' + ELEM)
						...
						if (device.ie && device.ie < 8) {
							return textArea.removeClass('layui-hide').addClass(
									SHOW);
						}
					...
					Edit.prototype.sync = function(index) {
						...
						textarea
								.val(toLower(iframeWin[0].document.body.innerHTML));
					};
					...
					Edit.prototype.getSelection = function(index) {
						...
						return document.selection ? range.text : range
								.toString();
					};
					...
					var setIframe = function(editor, textArea, set) {
						...
						iframe
								...
								.on(
										'load',
										function() {
											var conts = iframe.contents(), iframeWin = iframe
													.prop('contentWindow'), head = conts
													.find('head'), style = $([
													'<style>',
													'*{margin: 0; padding: 0;}',
											...
											hotkey.apply(that, [ iframeWin,
													iframe, textArea, set ]); // 快捷键处理
											toolActive.call(that, iframeWin,
													editor, set); // 触发工具
					...
					, getWin = function(index) {
						var iframe = $('#LAY_layedit_' + index), iframeWin = iframe
								.prop('contentWindow');
					...
					, filter = function(body) {
						var iframeWin = this, iframeDOM = iframeWin.document;
					...
					, Range = function(iframeDOM) {
						return iframeDOM.selection ? iframeDOM.selection
								.createRange() : iframeDOM.getSelection()
								.getRangeAt(0);
					}
					...
					, getContainer = function(range) {
						return range.endContainer
								|| range.parentElement().childNodes[0]
					}

					// 在选区插入内联元素
					, insertInline = function(tagName, attr, range) {
						var iframeDOM = this.document, elem = document
					...
					, toolCheck = function(tools, othis) {
						var iframeDOM = this.document, CHECK = 'layedit-tool-active', container = getContainer(Range(iframeDOM)), item = function(
								type) {
							return tools.find('.layedit-tool-' + type)
						}

						if (othis) {
							othis[othis.hasClass(CHECK) ? 'removeClass'
									: 'addClass'](CHECK);
						}
					...
					, toolActive = function(iframeWin, editor, set) {
						var iframeDOM = iframeWin.document, body = $(iframeDOM.body), toolEvent = {
							// 超链接
							link : function(range) {
								var container = getContainer(range), parentNode = $(
										container).parent();
						...
						, click = function() {
							var othis = $(this), events = othis
									.attr('layedit-event'), command = othis
									.attr('lay-command');
							...
							if (command) {
								...
								toolEvent[events]
										&& toolEvent[events].call(this, range);
							}
						...
						var body = this, index = layer
								.open({
									...
									success : function(layero, index) {
										...
										form.render('radio');
										layero.find('.layui-btn-primary').on(
												'click', function() {
													layer.close(index);
													body.focus();
												});
					...
					// 添加外链
					, olimage = function(options, callback) {
						var body = this, index = layer
								.open({
									type : 1,
									id : 'LAY_layedit_olimage',
									area : '350px',
									shade : 0.05,
									shadeClose : true,
									moveType : 1,
					...
					, face = function(callback) {
						...
						var faces = function() {
							...
							layui.each(alt, function(index, item) {
								arr[item] = layui.cache.dir + 'images/face/'
										+ index + '.gif';
							});
							return arr;
						}();
						...
						return face.index = layer.tips(function() {
							...
							layui.each(faces, function(key, item) {
								content.push('<li title="' + key
										+ '"><img src="' + item + '" alt="'
										+ key + '"></li>');
							});
							return '<ul class="layui-clear">'
									+ content.join('') + '</ul>';
						}(), this, {
							tips : 1,
							time : 0,
							...
							success : function(layero, index) {
								...
								$(document).off('click', face.hide).on('click',
										face.hide);
							}
						});
					}
					...
					, tools = {
						...
						olimage : '<i class="layui-icon layedit-tool-link" title="图片外链" layedit-event="olimage">&#xe654;</i>',
